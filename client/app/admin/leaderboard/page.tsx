...
export default function AdminLeaderboardPage() {
    ...
    async function load() {
      try {
        const [collegeRows, departmentRows, coderRows, engagingRows] = await Promise.all([
          getStudentLeaderboard({ size: 20 }),
          getDepartmentLeaderboard(),
          getBestCoders({ size: 20 }),
          getTopEngagingStudents({ size: 20 })
        ]);
  ...
  return (
    <AppShell expectedRole="SUPER_ADMIN" title="Leaderboard">
      <PageHeader title="Leaderboard" subtitle="Institution-wide student and department rankings." actions={<BackButton fallbackHref="/admin/dashboard" />} />
